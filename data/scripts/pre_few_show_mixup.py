...
def read_img_label(img_url, label_url, pre_label_url):
    ...
    _pre_label = pd.read_csv(header=None, names=['cls', 'x', 'y', 'w', 'h', 'iou'], sep=' ',
                             filepath_or_buffer=pre_label_url)
...
if __name__ == '__main__':
    ...
    pre_box_root = Path(fs_root / 'pre_labels')  # 预测框的label地址
    ...
    for img_name in tqdm(os.listdir(img_root)):
        ...
        for _, row in pre_label.iterrows():
            ...
            try:
                ...
                # patch = Image.blend(patch, target_patch, 1 - row['iou'])  # 预测对象叠加真实对象
                # patch = Image.blend(patch, target_patch, 0)  # 预测对象叠加真实对象
                image_out.paste(Image.blend(patch, target_patch, 1 - row['iou']), (int(xl), int(yt)))
            except Exception as e:
                print(f'损失一副图像拼接:[{img_name}], {e}')  # 本图中不包含所需的cls
